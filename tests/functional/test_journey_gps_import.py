...
Intent: a GPS file becomes exactly one flight (the unified Flight row
carries both the airframe and pilot-log data) + one track, and importing
it twice — confirming both times, not just the second time discarded —
...
_gps_import_create_segment (app/aircraft/routes.py) for the Flight
...
def test_gps_import_round_trip_no_duplicates(owner_env, app):
    ...
    with app.app_context():
        flight = Flight.query.filter_by(aircraft_id=aircraft_id).one()
        assert flight.pic_user_id is not None
    ...
    with app.app_context():
        flight_before = Flight.query.filter_by(aircraft_id=aircraft_id).one()
        counters_before = (
    ...
    with app.app_context():
        old_track_id = (
            Flight.query.filter_by(aircraft_id=aircraft_id).one().gps_track_id
        )

    # Confirm as-is (not skip) — the matched-flight path must update the
    # existing Flight/GpsTrack in place rather than creating a second copy
    # of either.
    submit(
        client,
        f"/aircraft/{aircraft_id}/gps-import/confirm-one",
    ...
    with app.app_context():
        ...
        new_track_id = flight.gps_track_id
    ...
    with app.app_context():
        flight_after = Flight.query.filter_by(aircraft_id=aircraft_id).one()
        counters_after = (
