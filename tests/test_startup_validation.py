...
class TestMaxUploadBytes:
    def test_non_integer_raises(self, monkeypatch):
        monkeypatch.setenv("MAX_UPLOAD_BYTES", "50MB")
        monkeypatch.delenv("BACKUP_ENCRYPTION_KEY", raising=False)
        with pytest.raises(
...
class TestBackupEncryptionKey:
    ...
    def test_real_key_passes(self, monkeypatch):
        monkeypatch.setenv("BACKUP_ENCRYPTION_KEY", "realkey")
        _validate_config(_app_with(GOOD))
...
class TestMultipleErrors:
    def test_all_errors_reported_together(self, monkeypatch):
        monkeypatch.setenv("FLASK_ENV", "production")
        monkeypatch.setenv("BACKUP_ENCRYPTION_KEY", "   ")
        monkeypatch.setenv("MAX_UPLOAD_BYTES", "notanumber")
        app = _app_with(
            {
                "SECRET_KEY": "short",
                "MAX_CONTENT_LENGTH": 50 * 1024 * 1024,
        ...
        assert "BACKUP_ENCRYPTION_KEY" in msg
